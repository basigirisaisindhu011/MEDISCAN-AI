    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-900 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-blue-900 via-slate-900 to-black p-6">
      <div className="w-full max-w-md rounded-3xl bg-white/95 backdrop-blur-xl p-8 shadow-2xl border border-white/20">
        <div className="flex flex-col items-center text-center">
          ...
          <p className="mt-2 text-sm text-slate-600">Start scanning prescriptions with MediScan AI</p>
        </div>

        {message && (
          <div className={`mt-6 rounded-2xl p-4 text-sm font-medium border ${message.includes('successful') ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-red-50 text-red-700 border-red-200'}`}>
            {message}
          </div>
        )}
        ...
        <p className="mt-6 text-center text-sm text-slate-600">
          Already have an account? <Link className="font-semibold text-blue-600 hover:underline" to="/login">Sign In</Link>
        </p>
      </div>
